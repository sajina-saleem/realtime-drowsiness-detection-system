...
root.title('Drwsiness Detection & Monitoring System')
label = Label(frame)
label.pack(side=TOP)
...
def start():
    ...
    def playAlarm(soundfile):
        sound_file = vlc.MediaPlayer(
            "file:///C:/Users/sajin/OneDrive/Desktop/project/Drowsiness-detection-master/alarm.wav")
        sound_file.play()


    def playAlarm1(soundfile):
        sound_file = vlc.MediaPlayer(
            "file:///C:/Users/sajin/OneDrive/Desktop/project/Drowsiness-detection-master/alarm1.wav")
        sound_file.play()
    ...
    while True:
        ...
        for rect in rects:
            ...
            if mouEAR > MOU_AR_THRESH:
                if not ALARM_ON:
                    ALARM_ON = True
                    t = Thread(target=playAlarm1, args=('alarm1.wav',))
                    t.daemon = True
                    t.start()
            ...
            cv2.putText(frame, "MOR: {:.2f}".format(mouEAR), (480, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.putText(frame, "GROUP-14 S8 CSE", (370, 470), cv2.FONT_HERSHEY_COMPLEX, 0.6, (138, 43, 226), 1)
        # show the frame
...
but3=Button(frame,padx=5,pady=5,width=18,bg='black',fg='grey',relief=GROOVE,command=start,text='START MONITORING',font=('helvetica 15 bold'))
but3.place(x=150,y=200)

but4 = Button(frame, padx=5, pady=5, width=5, bg='black', fg='grey', relief=GROOVE, text='EXIT', command=exitt,
              font=('helvetica 15 bold'))
but4.place(x=220, y=490)


but5=Button(frame,padx=5,pady=5,width=18,bg='black',fg='grey',relief=GROOVE,command=webrec,text='MONITOR & RECORD',font=('helvetica 15 bold'))
